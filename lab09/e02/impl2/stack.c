...
/*
 *  print all the stack elements (LIFO policy) in INVERSE order
 */
void stack_print_inverse(FILE *fp, stack_t *sp, void (*print)(FILE *, void *))
{
  if (sp != NULL) {
    stack_print_inverse_r(fp, sp->head, print);
  }

  return;
}

void stack_print_inverse_r(FILE *fp, node_t *node, void (*print)(FILE *, void *))
{
  if (node == NULL) {
    return;
  }

  stack_print_inverse_r(fp, node->next, print);
  print(fp, node->data);
  fprintf(fp, "\n");
